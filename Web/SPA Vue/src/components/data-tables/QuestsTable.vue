<template>
  <div>
    ...
    <v-data-table :items="getQuests" :headers="headers" :search="search">
      ...
      <template v-slot:item.type="{ item }">
        <v-chip
          small
          :color="questTypeChipColor(item.type)"
...
export default {
  ...
  computed: {
    ...mapGetters('quest', ['getQuests', 'getQuestById'])
  },
  created() {
    this.$store.dispatch('quest/fetchQuests');
  },
  methods: {
    ...mapMutations('quest', [ADD_QUEST, DELETE_QUEST, UPDATE_QUEST]),
    deleteItem(quest) {
