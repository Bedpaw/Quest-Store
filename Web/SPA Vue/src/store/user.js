...
import {
  ADD_USER,
  DELETE_USER,
  FETCH_LOGGED_USER, FETCH_MENTORS, FETCH_STUDENTS,
  FETCH_USERS,
  PUSH_ARTIFACT,
  UPDATE_USER
...
export const user = {
  ...
  mutations: {
    ...
    [ADD_USER]: (state, {user, userArray}) => {userArray.push(user)},

    [UPDATE_USER]: (state, {user, userArray}) => arrayUtils.updateItem(userArray, user),

    [DELETE_USER]: (state, {user, userArray}) => arrayUtils.removeItem(userArray, user),
  ...
  actions: {
    ...
    fetchStudents: storeActions.fetchResources(api.getStudents, FETCH_STUDENTS, Student),
    fetchMentors: storeActions.fetchResources(api.getMentors, FETCH_MENTORS, Mentor),
